...
for isession= par.session
    ...
    % restrict to certain trials
    if ~isempty(par.UStrials) 
    US.int= US.int(ismember(stim.session.fullcond.US_trials_cat, par.UStrials));
    US.trialIds.all= US.trialIds.all(:,ismember(stim.session.fullcond.US_trials_cat, par.UStrials));
    end
...
%     US.trialIds.m(:,1)= []; % remove first trial for same number of trials as others
    ...
    US.cellIds.l= find(sum(US.trialIds.l,2) >= par.minRespTrial);
    US.cellIds.m= find(sum(US.trialIds.m,2) >= par.minRespTrial);
    US.cellIds.h= find(sum(US.trialIds.h,2) >= par.minRespTrial);
    ...
    venn([length(US.cellIds.l) length(US.cellIds.m) length(US.cellIds.h)],... % venn([c1 c2 c3],...
        [length(US.cellIds.lm) length(US.cellIds.lh) length(US.cellIds.mh)  length(US.cellIds.lmh)]);  % [i12 i13 i23 i123])
    hold on
    % appearance
    % circles properties
    ...
    % name
    clear name T; name{1}= ['session ' num2str(isession) ' ' stim.session.session_labels{isession} [' trials' num2str(par.UStrials)]];
    name{2}= ['US-responsive cells to at least ' num2str(par.minRespTrial) ' of ' num2str(length(US.int))/3 ' stimuli'];
    ...
    T{ifield+2}= [num2str(size(US.trialIds.all,1)) ' total cells'];
