...
for isession= sessions %%%%%%%%%%%%
        ...
        if length(info.timestamps)>1 && ismember(isession,stim.session.sensoryStim)
            for itest= find(~cellfun(@isempty,info.timestamps))  % 2:length(info.timestamps)
        ...
        if ~contains(folder.parentDir{stim.group}, 'behavior only')
            ...
            elseif par.dfof
                F0= prctile(traces(par.range,:),par.dfof_pctile,1);
                for icell= 1:size(traces,2)
                    traces(:,icell)= (traces(:,icell)-F0(icell))/F0(icell);
                end
            end
        else
            traces=[]; spikes=[];
