...
if contains(folder.parentDir{1}, 'fullcond') % if fourth session to align
    nsession= 3;
else
    nsession= 4;
end
...
for imouse= stim.mice
    ...
    % exception
    if imouse==3 && contains(folder.parentDir{1}, 'fullcond') 
        ov23(end,:)=[];
    end
    ...
    % find position of cell ids that overlap with other sessions
    ...
    out= [unique(temp),histc(temp(:),unique(temp))]; % count number of cell ids occurence for each mouse
    out(out(:,2)==1,:)=[]; % if we want overlap with 2 other sessions, remove cells that overlap only with one
    samepos23= out(:,1); % ov23 positions to keep, with corresponding ids in session 1 and 4

    % find cell ids corresponding to position
    glob2= ov23(samepos23,1);
    glob3= ov23(samepos23,2);

    % find cell ids session 1 corresponding to global cell session 2
    [~,pos1]= ismember(glob2, ov12(:,2));
