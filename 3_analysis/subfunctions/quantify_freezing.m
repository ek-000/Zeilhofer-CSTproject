...
% add SEM for plot
plotdatSEM= squeeze(std(pctToneFreez,[],2))'; 
plotdatSEM=plotdatSEM./sqrt(size(pctToneFreez,2));
plotdatSEM= reshape(plotdatSEM,[1 size(plotdatSEM,1)*size(plotdatSEM,2)]);

if par.plot
    figure;
    b= bar(plotdat); hold on
    e= errorbar(plotdat,plotdatSEM,'.','color','k'); hold on
    e.YNegativeDelta=[]; % remove negative error bar
    % change color per session
    b.FaceColor = 'flat';
    for isession= par.sessions
