...
elseif ~isempty(par.per_US_intensity) && length(par.session_compare)>=1 % compare US intensities and sessions
    ...
    if ~isempty(par.UStrial_compare{1}) && length(par.session_compare)>=1  % compare sessions and selected trials
        mm= mm(~isnan(sum(mm,2)),:); % remove session with nans
         for isession= 1:length(par.session_compare)
            dat{isession}= mm(isession, ismember(stim.session.fullcond.US_trials_cat, par.UStrial_compare{itrial_compare}));
         end
    else
...
% % %% 2 ways ANOVA: sig (5 8 7) vs unsig (2 3 4)
% dat_veh=    dat; % 1=unsig, 2=sig
% dat_cno=    dat;
% stop here
% par.saveDir= 'C:\Users\ekamal\Desktop\New folder'; mkdir(par.saveDir)
% figure
% dat= [dat_veh dat_cno];
...
% vals= [[dat_veh{1}, dat_veh{2}] ; [dat_cno{1}, dat_cno{2}]];
% % column = unsig vs sig  row= veh vs CNO  
% [p, table]= anova2(vals', size(vals,2)/2); close
% clear name; name{1}= 'column = unexp vs exp  row= veh vs CNO';
% name{2}= ['column (unsig vs sig) P= ' num2str(round(p(1),4)) ' row (veh vs CNO  ) P= ' num2str(round(p(2),4)) ' interaction P= ' num2str(round(p(3),4))];
