...
for isession= sessions
        ...
        if ismember(isession,stim.session.sensoryStim)
            for itest= 1:size(dat{isession},3)
                try
                    ...
                    sortIds= stim.(par.datFields).(par.stim).sortIds{isession}; % in case sorted according to 1 test, all sortIds are the same
                end
    ...
    elseif ismember(isession,stim.session.sensoryStim)
        ...
        % percent positively responsive neurons
        respcellsNum= cellfun(@length, stim.traces.sensoryStim.respNegCellIds);
        respcellsPC= respcellsNum(isession,:)./stim.session.ncell(isession)*100;
        text(ts_bet+10, repmat(yl(2)+yl(2)/10,[1 length(ts_bet)]),  num2cell(round(respcellsPC,1))  ,'fontsize',10, 'fontweight', 'bold')
        
        % percent negatively responsive neurons
        respcellsNum= cellfun(@length, stim.traces.sensoryStim.respCellIds);
        respcellsPC= respcellsNum(isession,:)./stim.session.ncell(isession)*100;
        text(ts_bet+10, repmat(yl(2)+yl(2)/13,[1 length(ts_bet)]),  num2cell(round(respcellsPC,1))  ,'fontsize',10, 'fontweight', 'bold')
