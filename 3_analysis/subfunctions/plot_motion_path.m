function [mean_speed, total_motion]= plot_motion_path(data,stim,par)
...
for imouse= stim.mice
    ...
    if par.plot_path
        for isession= par.session
            ...
            mean_speed(imouse,isession)= (sum(motion) / length(motion)) *60; %%%%% output, cm/min
            total_motion(imouse,isession)= sum(motion) / stim.fr;   %%%%% output
            ...
            name{3}= [num2str(round(mean_speed(imouse,isession),1)) ' cm per min, ' num2str(round(total_motion(imouse,isession))) ' cm total'];
            title(name,'interpreter','none');
            clear name
            name= [stim.groupName ' mouse' num2str(imouse) ' session' num2str(isession) ' centroid position'];
            subtitle(name);
            % par.save
            if par.save
...
%% plot total_motion_compare
if par.compare_total_motion
    figure;
    plotdat= {total_motion(:,par.total_motion_sessions(1)), total_motion(:,par.total_motion_sessions(2))};
    yl= boxplots(plotdat,'mean','color', stim.session.sessioncols,'line','scatter','marker_alpha',0.5);
    SD_figure_appearance
    ylim([0 yl(2)])
    xticklabels(stim.session.session_labels(par.total_motion_sessions));
    ylabel('total distance traveled (cm)')
    % statistics
    clear name
    name{1}= 'Total distance traveled';
    str= SD_ttest(plotdat{1},plotdat{2});
    name{2}= str{1}; name{3}= str{2};
    title(name,'fontsize',12,'interpreter','none'); % update title if par.save
    if par.save
