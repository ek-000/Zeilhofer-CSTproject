...
%% average test data per trial
 for istim= stims %%%%%%%%%%%%%%%%%%
     meanTrialResponse={}; witstore={}; resppcCat={}; resppcCat_neg={}; respidsstore_neg={}; respidsstore={}; respIdsCat={}; respIdsCat_neg={}; sortIdsCat={}; respi={};  % temporary outputs (to append in struct within function)
...
for idatfield= 1:length(par.datFields)
        ...
        for isession= sessions
            ...
            mmcat=[]; clear respids respids_neg;
            ...
            for imouse= stim.mice
                if ~isempty(data(imouse,isession).(par.datFields{idatfield})) || contains(par.stims{istim},'motion')
                    ...
                    if strcmp(par.stims{istim},'CS') || strcmp(par.stims{istim},'motion_CS') % CS  or motion (motion in response to CS)
                        ts= data(imouse,isession).info.timestamps(1); ts{1}(ts{1}==0)=[]; % remove emtpy timestamps (fullcond)
                        if contains(folder.parentDir{1},'fullcond') && isession==2 
                            ts{1}= ts{1} - 2/stim.fr;
                        end
                    elseif strcmp(par.stims{istim},'US') || strcmp(par.stims{istim},'motion_US') % US or motion (motion in response to US)
                        ts= data(imouse,isession).info.timestamps(2); ts{1}(ts{1}==0)=[]; % remove emtpy timestamps (fullcond)
                        if isession==4 && contains(folder.parentDir{1},'SD047')  % exception, misaligned ts, use the ones form other days (same ons for US)
                             ts= data(imouse,isession-1).info.timestamps(2);
                        end
                        if ~contains(folder.parentDir{1},'fullcond')
                            ts{1}= ts{1}(:) - 1/stim.fr; % remove one frame, missaligned otherwise
                        elseif contains(folder.parentDir{1},'fullcond') && strcmp(par.stims{istim},'motion_US')
                                 ts{1}= ts{1}(:) - 2/stim.fr; % remove one frame, missaligned otherwise
                        end
                    ...
                    for itest= test_order
                        if ~isempty(ts{itest})
                            ...
                            if ismember(isession,stim.session.conditioning)
                                ...
                                elseif strcmp(par.stims{istim},'loco')
                                    trials_selec= par.trials{istim};
                                elseif strcmp(par.stims{istim},'motion') || contains(folder.parentDir{1},'fullcond')
                                    trials_selec= 1:length(ts{1}); % include all trials
                                end
                            else
                                trials_selec= par.trials{istim};
                            end
...
%                         cellIds{istim,imouse,isession}= 1:size(testresp,1);
                        cellIds{istim,imouse,isession}= 1:size(mm,1);
                    else
                        cellIds{istim,imouse,isession}=  lastId + (1:size(mm,1));
                    end

                    % append mouse response
                    mmcat= cat(1,mmcat,mm); % imouse, frames, itest, itrial
            ...
            if length(dims)==3 % only one test per session
                ...
                respWin= mmcat(:,twin_respp{idatfield},:);
                basWin= mmcat(:,1:twin_respp{idatfield}(1)-1,:);

                clear P
                for icell= 1:size(basWin,1)
                    for itrial=1:size(basWin,3)
                        if mean(respWin(icell,:,itrial)) > mean(basWin(icell,:,itrial))
                        [~,P(icell,itrial)] = ttest2(respWin(icell,:,itrial),basWin(icell,:,itrial)); 
                        else
                            P(icell,itrial)= 1;
                        end
                    end
                ...
                % decide if trial is responsive
                if par.pval
                    respids= P < par.Pval;
                elseif par.nresptrials
                    respids=        respi{isession} > par.thres_resp(istim);
                    respids_neg=    respi{isession} < -par.thres_resp(istim);
                end
                ...
                resppcCat_neg{isession,idatfield}=  round(100*sum(respids_neg(:,par.trials{istim}),'all') / (size(mmcat,1)*length(par.trials{istim})),1);
            ...
            elseif length(dims)==4  % several tests per session
                for itest= 1:dims(3)
                    ...
                    respids_neg(:,itest,:)= respi{isession}(:,itest,:) < -par.thres_resp(istim);
                end
                ...
                resppcCat_neg{isession,idatfield}=  round(100*sum(respids_neg(:,:,par.trials{4}),[1 3]) / (size(mmcat,1)*length(par.trials{4})),1);
            end
            ...
            if ~par.no_withdrawal
                ...
                respidsstore_neg{isession,idatfield}= respids_neg;
                if ismember(isession,stim.session.conditioning)
                    respIdsCat{isession}= find(sum(respids,2)>= par.minRespTrials(istim));
                    respIdsCat_neg{isession}= find(sum(respids_neg,2)>= par.minRespTrials(istim));
                elseif ismember(isession,stim.session.sensoryStim)
                    ...
                    [cellid_neg,testid_neg]= find( squeeze(sum(respids_neg,3)) >= par.minRespTrials(istim) );
                    for itest= 1:length(unique(testid_neg))
                        respIdsCat_neg{isession,itest}= cellid_neg(testid_neg==itest);
                    end
                end
            else
                respidsstore{isession,idatfield}= nan;
                respidsstore_neg{isession,idatfield}= nan;
                respIdsCat{isession}= nan;
                respIdsCat_neg{isession}= nan;
            end
            ...
            if par.cell_sort
                ...
                if  par.cell_sort_test && isequal(sessions,stim.session.sensoryStim)  % sort cells within every test
                    ...
                    if length(dims)==4 % sensory stim, several tests per session
                        for itest= 1:size(mmcat,3)
                            ...
                            else % sort according to mean response
        ...
        stim.(par.datFields{idatfield}).(par.stims{istim}).PcTrialNegResponses= resppcCat_neg(:,idatfield)'; % percent of responses per test/session (bad name)
        ...
        stim.(par.datFields{idatfield}).(par.stims{istim}).respNegCellIds= respIdsCat_neg; % inds responsive cells
