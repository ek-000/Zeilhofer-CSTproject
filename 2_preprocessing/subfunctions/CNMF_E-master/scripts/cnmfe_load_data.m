if and(ssub==1, tsub==1)
    neuron = neuron_full;
    Y = single(data.Y(:, :, sframe+(1:num2read)-1));
%     Y = double(data.Y(:, :, sframe+(1:num2read)-1));
    ...
    if exist(nam_mat, 'file')
        [Y, neuron_ds] = single(neuron_full.load_data(nam_mat, sframe, num2read));
%                 [Y, neuron_ds] = neuron_full.load_data(nam_mat, sframe, num2read);
        ...
        [Y, neuron_ds] = single(neuron_full.load_data(nam, sframe, num2read));
