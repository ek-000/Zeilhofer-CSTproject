...
% clear; clc; close all;
addpath(genpath('K:\_Simon\analysis'))

dir= 'K:\_Simon\data\SD013\SO 0405 L\2021_09_21';
filename= 'session_stack.mat';

tic
%% choose data
cd(dir)
neuron = Sources2D();
nam = ['./' filename]; % './data_1p.tif'          % this demo data is very small, here we just use it as an example
nam = neuron.select_data(nam);                      % if nam is [], then select data interactively

%% parameters
% -------------------------    COMPUTATION    -------------------------  %
pars_envs = struct('memory_size_to_use', 24, ...   % GB, memory space you allow to use in MATLAB
...
% -------------------------      TEMPORAL     -------------------------  %
Fs   = 10;          % frame rate
tsub = 1;           % temporal downsampling factor % 1
...
deconv_flag = false;     % run deconvolution or not % true
...
min_pnr = 12;       % minimum peak-to-noise ratio for a seeding pixel %8
