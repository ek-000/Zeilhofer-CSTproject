...
brightness_factor= 2.5; % increase behavior movie brightness
...
% smoothen a bit
% mean_fluo_filt= filtfilt(filtering, 1, mean_fluo);
...
plot(t,mean_fluo_filt,'k'); hold on
% appearance
...
if ~contains(test_dat.info.test,'spont')
    line([behavior.stim_start_sec behavior.stim_start_sec], [min(mean_fluo_filt) max(mean_fluo_filt)], 'color', 'r')
    % "withdrawal" text mark
    str= repmat('.',[length(behavior.stim_start_sec) 1]); % no withdrawal
    str(behavior.paw_withdrawal==1)= 'W';  % withdrawal
    text(behavior.stim_start_sec, repmat(max(mean_fluo_filt),[length(behavior.stim_start_sec) 1]), str)
...
elseif contains(experiment,'conditioning')
    if not(isempty(behavior.tone.tson))
    ...
    if ~isempty(behavior.US.tson)
        line([behavior.US.tson' behavior.US.tson'], [min(mean_fluo_filt) max(mean_fluo_filt)], 'color', 'r')
    end
end
% create dynamic line
l= line([0 0], [min(mean_fluo_filt) max(mean_fluo_filt)], 'color', 'b');
